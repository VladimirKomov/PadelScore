...
3. Проверьте параметры режима. Нажмите строку параметра и выберите значение из
   крупного списка; параметры `ON/OFF` меняются касанием строки.
4. Нажмите `START`.
...
- `GAME SCORING` — способ завершения гейма:
  - `STAR · 2 ADV` — после 40:40 показываются `DEUCE 1`, первое преимущество,
    `DEUCE 2`, второе преимущество, а после следующего возврата к равенству —
    `STAR POINT`. Следующий розыгрыш решает гейм;
  - `SILVER · 1 ADV` — допускается одно преимущество. Если оно не реализовано
    и счёт снова становится равным, показывается `SILVER POINT`, а следующий
    розыгрыш решает гейм;
  - `GOLDEN · 0 ADV` — первый розыгрыш после 40:40 сразу решает гейм;
  - `ADV · UNLIMITED` — обычная игра до перевеса в два очка без ограничения
    числа преимуществ.
- `SET ENDING` — единый выбор правила окончания сета:
  - `TB 6:6` — при равном счёте, соответствующем значению `GAMES`, начинается
    тай-брейк до 7 с перевесом в два очка;
  - `2-GAME LEAD` — тай-брейка нет, сет продолжается до перевеса в два гейма;
  - `FIRST TO 6` — сет заканчивается сразу при достижении значения `GAMES`,
    даже с перевесом в один гейм.
- `TRACK SERVE` — показывать подающую команду и автоматически передавать
  подачу после каждого завершённого гейма.
- `FIRST SERVER` — команда первой подачи: `A` или `B`.


Число в названиях `TB 6:6` и `FIRST TO 6` меняется вместе с параметром
`GAMES`. Эти варианты взаимоисключающие, поэтому невозможна противоречивая
комбинация правил. Стандартные настройки нового `CLASSIC`: два выигранных сета,
шесть геймов, `STAR` и тай-брейк при 6:6. В Star Point приложение сообщает
момент решающего розыгрыша, но выбор стороны приёма остаётся за игроками.
При тай-брейке подача меняется после первого очка, затем после каждых двух.
Первый гейм следующего сета подаёт команда, противоположная той, которая
начинала тай-брейк. Кнопка `SERVE` позволяет вручную исправить подающую
команду, а `UNDO` отменяет и такую корректировку.
...
- `POINTS` — общее число розыгрышей в одном раунде; после достижения этой
  суммы раунд заканчивается, даже если счёт равный. Доступные значения:
  16, 21, 24 или 32.
...
| Центральный статус | Одной крупной строкой показывает сокращённый режим и состояние матча: `CLASS · S0:0 · G2:1`, `CLASS · DEUCE 2`, `CLASS · ★ STAR`, `CLASS · ◇ SILVER`, `AM32 · R1 · 10 LEFT` или `TB7 · 3 LEFT` |
...
В варианте `STAR` центральная строка отдельно показывает `DEUCE 1`, `DEUCE 2`,
`ADV A/B` и `★ STAR`. В варианте `SILVER` после первого нереализованного
преимущества появляется `◇ SILVER`. В варианте `ADVANTAGE` используются более
привычные `DEUCE` и `ADV A/B`. Решающий Star или Silver Point выделяется
золотым цветом и двойной вибрацией, чтобы его было сложно пропустить во время
игры.
