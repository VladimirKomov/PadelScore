...
- `GAME SCORING` — способ завершения гейма:
  - `STAR` — после 40:40 показываются `DEUCE 1`, первое преимущество,
    `DEUCE 2`, второе преимущество, а после следующего возврата к равенству —
    `STAR POINT`. Следующий розыгрыш решает гейм;
  - `ADVANTAGE` — обычная игра до перевеса в два очка без ограничения числа
    преимуществ;
  - `GOLDEN` — первый розыгрыш после 40:40 сразу решает гейм.
...
| Центральный статус | Одной крупной строкой показывает сокращённый режим и состояние матча: `CLASS · S0:0 · G2:1`, `CLASS · DEUCE 2`, `CLASS · ★ STAR`, `AM32 · R1 · 10 LEFT` или `TB7 · 3 LEFT` |
...
`ADV A/B` и `★ STAR`. В варианте `ADVANTAGE` используются более привычные
`DEUCE` и `ADV A/B`. Star Point выделяется золотым цветом и двойной вибрацией,
чтобы решающий розыгрыш было сложно пропустить во время игры.
